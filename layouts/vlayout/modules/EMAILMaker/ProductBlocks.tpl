...
<div class="container-fluid" id="ProductBlocksContainer">
    ...
    <br>
    <label class="pull-left themeTextColor font-x-x-large">{vtranslate('LBL_PRODUCTBLOCKTPL','EMAILMaker')}</label>
    <br clear="all">{vtranslate('LBL_PRODUCTBLOCKTPL_DESC','EMAILMaker')}
    <hr>
    <br />
    <div class="row-fluid">
        <label class="fieldLabel"><strong>{vtranslate('LBL_DEFINE_PBTPL','EMAILMaker')}:</strong></label><br>    
        <div class="row-fluid">           
            <button type="button" class="btn btn-danger span1 marginLeftZero" onclick="confirm_delete();">{vtranslate('LBL_DELETE')}</button>    
            <div class="pull-right btn-group">
                <button type="submit" class="addProductBlock btn addButton marginLeftZero" data-url="?module=EMAILMaker&view=IndexAjax&mode=editProductBlock"><i class="icon-plus icon-white"></i>&nbsp;<strong> {vtranslate('LBL_ADD')}</strong></button>
        ...
        <div class="pushDownHalfper">    
            ...
            <thead>
                <tr class="blockHeader">
                    ...
                    <th style="border-left: 1px solid #DDD !important;" width="250px">{vtranslate('LBL_EMAIL_NAME','EMAILMaker')}</th>
                    <th style="border-left: 1px solid #DDD !important;" id="bodyColumn">{vtranslate('LBL_BODY','EMAILMaker')}</th>
                        ...
                        <div class="pull-right actions">
                            <div class="btn-group">
                                <button type="submit" class="btn marginLeftZero" onClick="this.form.tplid.value='{$tpl_id}'">
                                        <i title="Edit" class="icon-pencil alignBottom"></i></button><button class="btn marginLeftZero" <button type="submit" class="btn marginLeftZero" onClick="this.form.tplid.value='{$tpl_id}';this.form.mode.value='duplicate'">{vtranslate('LBL_DUPLICATE','EMAILMaker')}</button>
                            </div>
                        </div>
                    </td>
                </tr>
            {foreachelse}
                <tr id="noItemFountTr">
                    <td colspan="4" class="cellText" align="center" style="padding:10px;"><strong>{vtranslate('LBL_NO_ITEM_FOUND','EMAILMaker')}</strong></td>
                </tr>
            {/foreach}
            </tbody>
        </table>
